...
EOM
  ...
  if [[ -f "${GEN3_TFSCRIPT_FOLDER}/sample.tfvars" ]]; then
      while read line
      do
	if [[ $line == *"donotprint"* ]]; then
	  # For adding bash variables that can be reused in sample.tfvars.
	  eval $line
	elif [[ $line == *"$"* ]]; then
    	   # code to execute if string contains character
           eval echo "$line"
	else 
	   echo "$line"
       	fi
      done < "${GEN3_TFSCRIPT_FOLDER}/sample.tfvars"
